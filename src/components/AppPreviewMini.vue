    ...
    <section @mouseenter="inflate" @mouseleave="deflate" :class='isInflated ? "animated pulse" : "animated"'>
        <div class='card mt-3 mb-1 border rounded-0'>
            ...
            <div class="text-left pl-2 pr-2 pb-1">
                <i v-if="$attrs.data.keywords.includes('React')" title='React' class="fab fa-react fa-lg text-primary p-1"></i>
                <i v-if="$attrs.data.keywords.includes('Vue')" title='Vue' class="fab fa-vuejs fa-lg text-success p-1"></i>
                <i v-if="$attrs.data.keywords.includes('Angular')" title='Angular' class="fab fa-angular fa-lg text-danger p-1"></i>
                <i v-if="$attrs.data.keywords.includes('Node')" title='Node' class="fab fa-node fa-lg text-secondary p-1"></i>
                <i v-if="$attrs.data.keywords.includes('JavaScript')" title='JavaScript' class="fab fa-js fa-lg text-warning p-1"></i>
                <i v-if="$attrs.data.keywords.includes('Python')" title='Python' class="fab fa-python fa-lg text-info p-1"></i>
                <i v-if="$attrs.data.keywords.includes('Java')" title='Java' class="fab fa-java fa-lg text-danger p-1"></i>
                <i v-if="$attrs.data.keywords.includes('C++')" title='C++' class="fa-lg text-info p-1">C++</i>
            ...
            <div class="text-left p-2" style="padding-top: 0px !important">
                <router-link :to="{name:'Applications-Edit', params: {id: $attrs.data._id.$oid || $attrs.data._id}}">
